...
public class ProjectileMagicMissile : Projectile //마법사가 사용하는 투사체
{
    ...
    protected override IEnumerator Co_Shot() //투사체 날아가는 코루틴 재정의.
    {
        while (true)
        {
            ...
            if (Vector3.Distance(transform.position, targetPosByOffset) <= 0.1) owner.ReturnProjectile(this);

            //타겟이 사망한 경우의 처리
            if (!target.gameObject.activeSelf) owner.ReturnProjectile(this);
